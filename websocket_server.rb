...
module Listenable
  ...
  def emit(event_name, *args)
    listeners.each do |listener|
      listener[1].call(*args) if listener[0] == event_name
    end
  end
...
class WebSocket
  ...
  def handshake
    ...
    begin
      ...
      return close_fail('HTTP Request timeout (5s)')
    end

    return close_fail('HTTP Request was empty') if http_request == ''

    # validate request, but only HTTP version, we accept all uris
    matches = http_request.lines[0].match(%r{^GET (?<uri>/.*) HTTP/(?<version>\d\.\d)$})
    return close_fail('HTTP Version must be at least 1.1') if !matches || matches[:version].to_f < 1.1

    # we don"t really care what, it just must exist
    matches = http_request.match(/^Host:\s+(?<host>[^:]*)(:(?<port>[1-9]*))?$/)
    return close_fail("HTTP Request didn't have a Host header") if !matches || matches[:host] == ''
    ...
    if !matches || !matches[:upgrades].include?('websocket')
      return close_fail("HTTP Request didn't have websocket Upgrade header")
    end
    ...
    if !matches || !matches[:connections].include?('Upgrade')
      return close_fail("HTTP Request didn't have Connection header set to Upgrade")
    end

    # websocket version must be 13
    matches = http_request.match(/^Sec-WebSocket-Version:\s+(?<version>[1-9]+)$/)
    return close_fail("HTTP Request websocket version wasn't 13") if !matches || matches[:version].to_i != 13
    ...
    return close_fail("HTTP Request didn't have websocket key") if !matches || matches[:key] == ''
    ...
    begin
      ...
      return close_fail('Connection Failed!')
    end
  ...
  def recvheader
    ...
    return send_close(1002, 'Unmasked frame from client to server!') if is_masked == 0
    ...
    [fin, opcode, is_masked, payload_length, data]
  end

  def recv
    return if closed?

    fin, opcode, _is_maksed, payload_length, data = recvheader
    return if data.nil?
    ...
    when 1
      ...
      emit_thread('message_text', msg)
    when 2
      # binary frame
      msg = data
      emit_thread('message_binary', msg)
    when 8
      # close frame
      return send_close(1002, "Control frame can't be fragmented") if fin != 1

      if payload_length > 125
        return send_close(1002,
                          "Control frame can't have payload length greater than 125")
      end

      if @status == Status::OPEN && payload_length > 0
        status_code = data.pack('n').unpack1('n')
        msg = data.drop(2).pack('C*').force_encoding('utf-8')

        send_close(status_code, msg)
      else
        send_close(1005)
      end
    when 9
    ...
    when 10
      ...
      send_close(1002, 'Unsupported Opcode!')
    end
  end
  ...
  def send(fin, opcode, mask, data = [])
    ...
    data = data.is_a?(Array) ? data : data.codepoints
  ...
  def closed?
    @raw_socket.closed? || @status == Status::CLOSING || @status == Status::CLOSED
  end
...
class WebSocketServer
  ...
  def start
    until @server.closed?
      ...
      readable, = IO.select([@server, *@clients], nil, nil, 1)
      ...
      readable.each do |socket|
        if socket == @server
          ...
          ensure_one_connection(websocket)
          @clients.push(websocket)
          sleep(0.1)
          emit_thread('connection', websocket)
          next
        end
